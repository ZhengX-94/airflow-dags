...
SPARK_TEST_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                '--executor-memory','3g',
                '--executor-cores','2',
                '--py-files','s3://wcdde-midterm-demo/job.zip',
                's3://wcdde-midterm-demo/workflow_entry.py',
                '-p','''{"input_path": "{{ task_instance.xcom_pull('parse_request', key='s3location') }}",
                    "name": "class_demo",
                    "file_type": "txt",
                    "output_path": "s3://wcdde-midterm-output/output_folder/",
                    "partition_column": "job"}'''
                # 's3a://demo-wcd/WCD-demo_2.11-1.0.jar',
                # '-p'
                # '-p','wcd-demo',
                # '-i','Csv',
                # '-o','parquet',
